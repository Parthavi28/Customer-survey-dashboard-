...
import { value } from '../assets/jsonValue';

export default function BarAnimation({item}) {
  const [percentage, setPercentage] = useState([0,0,0,0,0])
  const [bar,setBar]=useState([])
    ...
    useEffect(() => {
        ...
        const getAnswer = () => {
            const data = item.text_question
            console.log("data: " + data)
            // api.post("/survey/getData",data).then((res) => {
            //     console.log("answer",res)
            // })
           fetch("http://localhost:5000/survey/getData", {
    ...
    .then((res) => {
      ...
      for (const key in res) {
        barname.push(res[key]);
      }
      setPercentage(barname)
    })
    .catch((error) => {
